...
execute if score $playing gugle_badapple matches 1 if score $render gugle_badapple matches 0..99 at @e[type=minecraft:marker,tag=gugle_badapple_origin,limit=1] run function gugle:dispatch/root
execute if score $playing gugle_badapple matches 1 if score $frame gugle_badapple matches 100.. run scoreboard players set $playing gugle_badapple 0
